unit Menu_Reservas;
interface
uses crt,Reservas,Pasajeros,Vuelos,dos,validacion,mensajes;

var arch:reserva; arch_p:pasajero; arch_v:vuelo;

procedure menu_reserva ();
...
begin
     textcolor (yellow);
     textbackground (blue);
     gotoxy(x,y); writeln  ('------------------------------------------------------------------ Reservas ----------------------------------------------------------------------');
     gotoxy(x,y+1); writeln('--------------------------------------------------------------------------------------------------------------------------------------------------');
     gotoxy(x,y+2); writeln('|           ID Reserva         |    ID Pasajero    |           ID Vuelo           |       Fecha       |     Hora     |           Costo           |');
     gotoxy(x,y+3); writeln('--------------------------------------------------------------------------------------------------------------------------------------------------');
end;

procedure si_existe_vuelo(var arch_v:vuelo; var reg:t_reserva; var pos2:integer; x,y:integer);
...
procedure si_existe_pasajero(var arch_p:pasajero; var reg:t_reserva; x,y:integer);
...
procedure mostrar_reservas (reg:t_reserva; var x,y:integer);
begin
     datos (x,y);
     with reg do
     begin
          datos (x,y);
          gotoxy(x,y+4); writeln('|');gotoxy(x+2,y+4); writeln(id_reserva);
...
procedure mostrar_reservas_listado (reg:t_reserva; p,p_2:integer);
var x:integer;
begin
     textcolor (yellow);
     textbackground (blue);
     ...
     begin
          ...
          begin
               ...
               gotoxy(x+2,p+5); writeln('|');gotoxy(x+4,p+5); writeln(id_reserva);
...
procedure Listado_reservas_5 (var arch:reserva; var op:char );
var reg:t_reserva; pos:integer; p,p_2:integer; x,y:integer;
begin
     p:=1;
     p_2:=0;
     pos:=0;
     ...
     begin
          textcolor (yellow);
          textbackground (blue);
          gotoxy(x,p+1); writeln  ('------------------------------------------------------------------ Reservas ----------------------------------------------------------------------');
          gotoxy(x,p+2); writeln('--------------------------------------------------------------------------------------------------------------------------------------------------');
          gotoxy(x,p+3); writeln('|           ID Reserva         |    ID Pasajero    |           ID Vuelo           |       Fecha       |     Hora     |           Costo           |');
          gotoxy(x,p+4); writeln('--------------------------------------------------------------------------------------------------------------------------------------------------');          if eof(arch) then
          begin
               gotoxy(x,p+5); writeln('                                                                                                                                                  ');
               gotoxy(x,p+5); writeln('|');gotoxy(x+60,p+5); writeln('No hay reservas para mostrar');
                    ...
                    begin
                         ...
                         begin
                            dec (p);
                             ...
                             inc (p_2);
                         mostrar_reservas_listado (reg,p,p_2);
                         inc(p);
                         y:=p;
                    end;
                    if eof (arch) then
...
procedure alta_reserva (var arch:reserva; var arch_p:pasajero; var arch_v:vuelo);
var id:integer; pos,pos2:integer; op:char; reg:t_reserva; cond:char; x,y:integer; reg_v:t_vuelo;
begin
     op:=' ';
     x:=5;
     y:=7;
     textbackground (blue);clrscr;
     abrir_reserva (arch);
     ordenar_reserva (arch);
     listado_reservas_5(arch,op);
     if op = '2' then
     begin
          repeat
          begin
          ...
          if op = '3' then
             ...
             begin
                  ...
                  buscar_reserva_id (arch,reg,pos,id);
                  if pos = (-1) then
                  begin
                  ...
                  begin
                       ...
                       begin
                            ...
                            begin
                                 ...
                                 textbackground (black); writeln ('Reserva existente:');
                                 mostrar_reservas(reg,x,y);
                       ...
                       begin
                            ...
                            begin
                                 textbackground (blue);clrscr;
                                 gotoxy (x,y-2);textcolor (white);
                                 textbackground (black); writeln ('Reserva dada de baja:');
                                 mostrar_reservas(reg,x,y);
...
procedure baja_reserva(var arch:reserva);
var id:integer; pos,pos2:integer; op:char; reg:t_reserva; cond:char; x,y:integer;  reg_v:t_vuelo;
begin
     x:=5;
     y:=7;
...
procedure modificar_reserva(var arch:reserva; var arch_v:vuelo; var arch_p:pasajero);
var id:integer; pos,pos2,pos3:integer; op:char; reg,reg_2:t_reserva; cond:char; minutos,dia,mes,anio,hora:integer; costo:real; x,y:integer; reg_v:t_vuelo;
begin
     x:=5;
     y:=7;
     textbackground (blue);clrscr;
                            ...
                            case op of
                                 ...
                                 '3':
                                     begin
                                          ...
                                          repeat
                                          begin
                                               gotoxy (x,y+17); writeln ('Desea confirmar la operacion? s/n: ');
                                               gotoxy (x+36,y+17); cond:=readkey;
                                          end;
                                          until cond in ['S','N','s','n'];
                                          if (cond = 'S') or (cond = 's') then
                                          begin
                                 ...
                                 '4':
                                     begin
                                          ...
                                          repeat
                                          begin
                                               gotoxy (x,y+15); writeln ('Desea confirmar la operacion? s/n: ');
                                 ...
                                 '5':
                                     begin
                                          gotoxy (x,y+11); writeln ('                                              ');
                                          gotoxy (x,y+11);textcolor (white);
                                          textbackground (black); writeln ('Costo:  '); validacion_real (costo,10,x+7,y+11);
                                          repeat
                                          begin
                                               gotoxy (x,y+13); writeln ('Desea confirmar la operacion? s/n: ');
                                               gotoxy (x+34,y+13); cond:=readkey;
                                          end;
                                          until cond in ['S','N','s','n'];
                                          if (cond = 'S') or (cond = 's') then
...
procedure consulta_reserva(var arch:reserva);
var op:char; reg:t_reserva; pos:integer; id:integer; x,y:integer;
begin
     x:=5;
     y:=7;
     textbackground (blue);clrscr;
...
procedure listado_reserva (var arch:reserva );
var reg:t_reserva; pos:integer; p,p_2:integer; x,y:integer; op:char;
begin
     abrir_reserva (arch);
     ordenar_reserva (arch);
     p:=1;
...
procedure borrar_archivo(var arch:reserva);
...
procedure menu_reserva();
var op:char; arch:reserva; x,y:integer;
begin
     x:=62;
     y:=8;
     clrscr;
     ...
     begin
          ...
          begin
            ...
            gotoxy (x+9,y+2);writeln('Menu Reservas');
          ...
          until op in ['1','2','3','4','5',#27];
          case op of
               '1':alta_reserva(arch,arch_p,arch_v);
               '2':baja_reserva(arch);
               '3':modificar_reserva(arch,arch_v,arch_p);
               '4':consulta_reserva(arch);
               '5':listado_reserva (arch);
